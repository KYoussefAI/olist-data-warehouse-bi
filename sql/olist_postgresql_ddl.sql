...
--
-- This script defines:
--   1. Raw landing tables that mirror the CSV files
--   2. Staging tables with cleaned and typed structures
--   3. Core warehouse dimensions and facts
--   4. Dashboard marts as views
...
SELECT
    ...
    SUM(foi.item_price) AS gross_item_value,
    SUM(foi.freight_value) AS gross_freight_value,
    SUM(foi.item_price + foi.freight_value) AS gross_revenue
FROM dwh.fact_orders fo
LEFT JOIN dwh.fact_order_items foi
...
LEFT JOIN dwh.dim_order_status os
    ON fo.order_status_key = os.order_status_key
GROUP BY
    dd.year_number,
    dd.month_number,
    dd.month_name,
...
SELECT
    ...
    AVG(
        EXTRACT(EPOCH FROM (
            fo.order_delivered_customer_ts - fo.order_purchase_ts
        )) / 86400.0
    ) AS avg_days_purchase_to_delivery,
    AVG(
        EXTRACT(EPOCH FROM (
            fo.order_delivered_customer_ts - fo.order_estimated_delivery_ts
        )) / 86400.0
...
LEFT JOIN dwh.fact_closed_deals fcd
    ON fml.mql_id = fcd.mql_id
LEFT JOIN dwh.dim_date dd
    ON fml.first_contact_date_key = dd.date_key
LEFT JOIN dwh.dim_marketing_origin mo
    ON fml.marketing_origin_key = mo.marketing_origin_key
GROUP BY
    dd.year_number,
    dd.month_number,
    dd.month_name,
